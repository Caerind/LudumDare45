...
#include <sstream> // Used for DateTime to string for screenshot
...
void Window::closeOnClosed(bool closeOperation)
{
	if (closeOperation && !mWindowCloseSlot.isConnected())
	{
		mWindowCloseSlot.connect(onWindowClosed, [this](const en::Window*) { close(); });
	}
	else if (!closeOperation && mWindowCloseSlot.isConnected())
	{
		mWindowCloseSlot.disconnect();
	}
}

bool Window::doCloseOnClosed() const
{
	return mWindowCloseSlot.isConnected();
}
